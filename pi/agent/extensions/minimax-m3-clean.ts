		...
		models: [
			{
				...
				reasoning: true,
				input: ["text", "image"],
				// MiniMax-M3 pricing (per 1M tokens) — permanent 50% off applied.
				// Tiered by INPUT context size:
				//   ≤ 512k input: input 0.30, output 1.20, cacheRead 0.06
				//   > 512k input: input 0.60, output 2.40, cacheRead 0.12
				// pi-ai's `cost` field is static, so we record the ≤ 512k tier
				// (the common case) and flag the > 512k tier here. Long-context
				// sessions will under-report cost.
